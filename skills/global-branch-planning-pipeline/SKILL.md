...
아래 경우에는 전체 흐름을 시작하지 않고, 필요한 단계만 수행한다.

- 계획과 리스크 검토만 필요함: `global-planning-pipeline`
- 이슈만 작성/생성하면 됨: 저장소 템플릿과 라벨을 확인한 뒤 이슈 단계만 수행
- 커밋만 하면 됨: 변경 범위와 저장소의 커밋 규칙을 확인한 뒤 커밋 단계만 수행
- PR만 만들면 됨: 현재 브랜치, 커밋, 이슈 연결 상태를 확인한 뒤 PR 단계만 수행
...
6. 이슈 제목, 본문, 라벨은 저장소 템플릿과 기존 이슈의 관례를 우선 따른다.
...
- 이 스킬에서 정한 issue, commit, PR 규칙과 어긋나는 독자 포맷을 새로 만들지 않는다.
