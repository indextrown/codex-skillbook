...
키트에서 제외된 문서는 재실행할 때 `RETIRED`로 표시하고 관리 목록에서만 제거해요. 파일 자체는 자동으로 삭제하지 않아요. 이전 키트가 만든 `docs/Root.md`도 필요한 내용을 확인한 뒤 직접 정리할 수 있어요.
...
| `RETIRED` | 키트에서 제외된 문서를 삭제하지 않고 관리 대상에서만 제거해요. |
