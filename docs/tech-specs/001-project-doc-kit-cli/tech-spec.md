...
- Git 흐름 선택 문서는 커밋 규칙 확인, PR 제목과 본문 작성, 저장소 PR 템플릿 사용 순서를 구체적으로 안내해야 해요.
...
커밋 규칙은 저장소 지침과 최근 커밋을 순서대로 확인하고, 규칙이 없다면 Conventional Commits를 제안 예시로 보여줘요. PR 본문은 대상 프로젝트의 `.github/PULL_REQUEST_TEMPLATE.md`와 대소문자 변형·템플릿 폴더를 먼저 확인해요. 템플릿이 없으면 자동으로 만들지 않고 사용자에게 생성 여부와 필요한 섹션을 물어요. 관련 이슈 입력란이 있어도 이슈 생성 권한으로 해석하지 않으며, 사용자가 명시적으로 요청한 경우에만 이슈를 만들어요. `main`, 브랜치 접두사, Conventional Commits와 릴리스 규칙은 확인 전까지 팀 정책으로 단정하지 않아요.
