...
RxSwift를 채택한 프로젝트는 의존성 선언에서 RxSwift·RxCocoa가 확인되면 타입·연산자, 바인딩 정책, Input/Output 패턴 문서 세 개를 자동으로 받아요. 자동으로 확인하기 어려운 구조에서는 `--include rxswift`로 직접 추가할 수 있어요. 생성된 정책 문서는 실제 구현과 팀의 합의를 확인한 뒤 적용해요.
...
- RxSwift를 쓰는 프로젝트는 의존성 선언만으로 서로 연결된 참고·정책 검토·Input/Output 문서 세 개를 받을 수 있어야 해요.
- 자동 감지되지 않은 RxSwift 문서는 `--include rxswift`로 직접 추가할 수 있어야 해요.
- 실행 결과에는 자동으로 포함한 근거와 제외한 선택 문서의 추가 옵션이 보여야 해요.
...
CLI는 `Package.swift`, `Package.resolved`, `project.pbxproj`, `Podfile`, `Podfile.lock`, `Cartfile`, `Cartfile.resolved`와 Tuist 의존성 선언처럼 이름이 정해진 파일만 확인해요. 이 파일에서 RxSwift 또는 RxCocoa를 찾으면 `rxswift` 선택 문서 세 개를 자동으로 포함해요. 심볼릭 링크, 빌드 산출물, Pods와 5MB를 넘는 선언 파일은 읽지 않아요. 자동 감지 근거가 된 상대 경로는 파일 계획 위에 표시해요. 감지하지 못한 프로젝트는 기존 `--include rxswift` 옵션을 사용해요.
...
파일 목록 위에는 자동 포함과 제외 이유를 함께 보여줘요. 예를 들어 RxSwift를 자동으로 찾으면 `자동 포함`과 근거 파일을 표시해요. Git 작업 흐름처럼 남은 선택 문서는 `선택 문서 제외`와 다시 실행할 `--include` 옵션을 표시해요. 따라서 기본 명령이 선택 문서를 빠뜨린 것처럼 보이지 않아야 해요.
...
RxSwift 의존성이 없는 가상의 UIKit 프로젝트 `MyUIKitApp` 루트에서 기본 명령을 실행하고 적용을 확인한 경우예요. 실행 전에는 다음 파일이 있다고 가정해요.
...
선택 문서 `rxswift.md`는 RxSwift·RxCocoa 타입, 구독 메서드, 연산자 비교와 최종 선택표를 담아요. 읽기 순서를 제시하고 나머지 두 문서로 연결해요. `rxswift-binding-policy.md`는 메모리, DisposeBag, Relay, UI 스레드와 Driver 사용 여부를 팀 정책 검토안으로 정리해요. `rxswift-input-output.md`는 Input을 먼저 만들고 UI 이벤트를 개별 바인딩하는 흐름, `transform`, View의 Binder, 대표 Input/Output 연결 예시를 담아요. 타입·화면 이름은 가상 UIKit 예시로 표시해요. `defaultTapThrottle()` 같은 별도 확장은 사용 전 확인 대상으로 표시해요. 기본 키트가 RxSwift 도입이나 Driver 미사용을 요구하지 않도록 의존성이 확인되거나 사용자가 선택한 경우에만 세 문서를 생성하고, 포함하지 않은 경우 길잡이에 깨진 링크를 만들지 않아요.
...
로컬 패키지는 `npm pack --dry-run`으로 실행 파일과 키트만 포함하는지 확인해요. 테스트는 `--dry-run`의 무변경, 확인 거절, 기본·선택 파일, RxSwift 의존성 자동 감지, 선택 문서 제외 안내, 사용자 수정 보존, 수정하지 않은 생성 문서 갱신, 관리 이력이 없는 문서 처리, 제외된 미수정 문서의 삭제와 수정 문서의 보존, 기존 `docs/`와의 공존, 경로·심볼릭 링크 거부와 `확인 필요` 표기를 검증해요. 기본 브랜치에 병합한 뒤에는 GitHub 저장소 주소로 임시 프로젝트에서 원격 실행을 확인해요. 이때 대상 프로젝트에 `package.json`, 잠금 파일이나 키트 저장소가 새로 생기지 않는지도 검사해요.

Git 참조를 생략한 원격 실행 흐름은 임시 폴더에서 검증했어요. 현재 문서 구성은 로컬 패키징 테스트에서 확인했어요. RxSwift를 찾지 못한 프로젝트에서 `--dry-run`은 파일을 만들지 않고 기본 여섯 파일을 표시하고, `--apply`는 기본 여섯 파일을 만들어요. RxSwift를 찾으면 세 문서를 자동으로 더 표시하고 만들어요. `gitflow`, 명시적인 `rxswift`, 두 옵션 동시 지정은 각각 7개, 9개, 10개 파일을 만들어요. 기본 브랜치에 병합한 뒤 같은 원격 실행을 다시 확인해요.
...
- RxSwift 자동 감지는 의존성 선언만 확인하고 Swift 소스 전체를 분석하지 않아요. 파일 이름이나 위치가 일반적이지 않으면 감지하지 못할 수 있으므로 `--include rxswift`를 계속 제공해요.
...
- [x] 6. 같은 명령을 이용한 안전한 갱신 구현
  ...
  - [x] 6-6. 키트에서 제외한 기존 관리 문서는 수정되지 않았으면 삭제하고, 수정됐으면 보존해요. 관리 기록이 먼저 제거된 이전 생성 문서도 정확한 템플릿 비교로 정리해요. 실행할 때 생략한 선택 문서는 계속 관리해요.
...
- [x] 9. DI Container 패턴 문서 추가
  ...
  - [x] 9-4. 기본 생성, 선택 문서 조합, 재실행과 기존 문서 보존 테스트가 통과해요.
...
- [x] 12. RxSwift 선택 문서 자동 감지와 실행 안내 개선
  - [x] 12-1. 의존성 선언 파일에서 RxSwift·RxCocoa를 찾으면 관련 문서 세 개를 자동으로 포함해요.
  - [x] 12-2. 자동 감지 근거 파일과 제외한 선택 문서의 `--include` 옵션을 실행 결과에 표시해요.
  - [x] 12-3. 자동 감지하지 못한 프로젝트를 위해 `--include rxswift` 동작을 유지해요.
  - [x] 12-4. MapseaLab과 같은 Xcode Swift Package Manager 경로와 기본·명시 선택 흐름을 회귀 테스트로 검증해요.
